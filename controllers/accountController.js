...
async function buildRegisterByAdmin(req, res, next) {
  let nav = await utilities.getNav()
  let accountTypes = await Account.getAccountTypes();
...
async function registerAccountByAdmin(req, res) {
  let nav = await utilities.getNav()
  ...
  try {
    ...
    hashedPassword = await bcrypt.hashSync(account_password, 10)
    console.log("Hashed Password:", hashedPassword)
    const regResult = await Account.registerAccountByAdmin(
      account_firstname,
      account_lastname,
      account_email,
      hashedPassword,
